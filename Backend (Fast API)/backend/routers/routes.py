...
from backend.schema import PelangganSchema, PemesananSchema, PelangganListSchema, PemesananListSchema
import backend.routers.crud as crud
from backend.configdb import SessionLocal
from backend.configdb import get_db
...
@router.post("/pemesanan/", response_model=PemesananSchema)
async def create_pemesanan(pemesanan: PemesananSchema, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    return crud.create_pemesanan(db, pemesanan, pemesanan.pelanggan_id)
